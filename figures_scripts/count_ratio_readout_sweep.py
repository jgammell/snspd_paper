...
figure_filename = 'count_ratio_readout_sweep'
width = 4
ratio = 1.618
height = width/ratio
fig, ax = plt.subplots()
fig.set_size_inches(width, height)
ax.set_xlabel('Readout current (μA)')
ax.set_ylabel('Ratio')
ax.set_xlim([0, 12])
ax.set_ylim([-.1, 1.1])
ax.set_title('Ratio of SNSPD count rate to readout count ratio\nvs. readout bias current')

# Import data
data_filepath = r'C:/Users/jgamm/Desktop/superconductivity_paper/data'
with open(os.path.join(data_filepath, 'D1_ratio_sweeping_readout_current_isnspd30u.pickle'), 'rb') as F:
    Dev1 = pickle.load(F)
    Dev1['ratio'] = [0 if Dev1['readout count rate'][i]==0 \
    ...
    Dev1['average ratio'] = np.mean(Dev1['ratio'] \
        [np.argmin(np.abs([i-2.5 for i in Dev1['readout bias']])) : \
         np.argmin(np.abs([i-7 for i in Dev1['readout bias']]))])
with open(os.path.join(data_filepath, 'D2_ratio_sweeping_readout_current_isnspd30u.pickle'), 'rb') as F:
    Dev2 = pickle.load(F)
    Dev2['ratio'] = [0 if Dev2['readout count rate'][i]==0 \
    ...
    Dev2['average ratio'] = np.mean(Dev2['ratio'] \
        [np.argmin(np.abs([i-2.5 for i in Dev2['readout bias']])) : \
         np.argmin(np.abs([i-7 for i in Dev2['readout bias']]))])
with open(os.path.join(data_filepath, 'D3_ratio_sweeping_readout_current_isnspd30u.pickle'), 'rb') as F:
    Dev3 = pickle.load(F)
    Dev3['ratio'] = [0 if Dev3['readout count rate'][i]==0 \
    ...
    Dev3['average ratio'] = np.mean(Dev3['ratio'] \
        [np.argmin(np.abs([i-2.5 for i in Dev3['readout bias']])) : \
         np.argmin(np.abs([i-7 for i in Dev3['readout bias']]))])
with open(os.path.join(data_filepath, 'D4_ratio_sweeping_readout_current_isnspd30u.pickle'), 'rb') as F:
    Dev4 = pickle.load(F)
    Dev4['ratio'] = [0 if Dev4['readout count rate'][i]==0 \
    ...
    Dev4['average ratio'] = np.mean(Dev4['ratio'] \
        [np.argmin(np.abs([i-2.5 for i in Dev4['readout bias']])) : \
         np.argmin(np.abs([i-7 for i in Dev4['readout bias']]))])

...
figure_filename = 'readout_bias_sweep'
width = 4
ratio = 1.618
height = width/ratio
fig, ax = plt.subplots()
...
ax.set_title('Readout wire: count rate vs. bias current')

# Import data
data_filepath = r'C:\Users\jgamm\Documents\GitHub\snspd_paper\data'
with open(os.path.join(data_filepath, 'D1_readout_bias_sweep.pickle'), 'rb') as F:
    Dev1 = pickle.load(F)
with open(os.path.join(data_filepath, 'D2_readout_bias_sweep.pickle'), 'rb') as F:
    Dev2 = pickle.load(F)
with open(os.path.join(data_filepath, 'D3_readout_bias_sweep.pickle'), 'rb') as F:
    Dev3 = pickle.load(F)
with open(os.path.join(data_filepath, 'D4_readout_bias_sweep.pickle'), 'rb') as F:
    Dev4 = pickle.load(F)
